...
const Artical = ({ articalPost}) => {
  console.log(articalPost);
  
  
  const date = new Date(articalPost.date).toLocaleDateString("ar-EG", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  return (
    ...
    >
      <Link className="block" to={`/blog/${articalPost.slug}`}>
        <div className="grid md:grid-cols-2 gap-0">
          {/* Image */}
          <div className="relative h-72 md:h-100 overflow-hidden">
            <img
              alt={articalPost.title}
              className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out"
              src={articalPost.image}
            />
