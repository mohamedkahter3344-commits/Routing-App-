...
const BlogDetails = ({ articalPost, viewMode }) => {
  const { slug } = useParams();

  const postChoose = articalPost.find((post) => post.slug === slug);
  // console.log(postChoose);
  const arrayBlogDetail = [...articalPost]
    .sort(() => Math.random() - 0.5)
    .slice(0, 3);

  const date = new Date(postChoose.date).toLocaleDateString("ar-EG", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
    ...
    <main className="flex-grow pt-20">
      <article className="bg-[#0a0a0a] min-h-screen">
        ...
        <div className="relative h-[60vh] min-h-[500px] overflow-hidden">
          <img
            alt={postChoose.title}
            className="absolute inset-0 w-full h-full object-cover"
            src={postChoose.image}
          />
            ...
            <div className="max-w-5xl mx-auto">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <Link
                  ...
                  to={`/blog?category=${postChoose.category}`}
                >
                  {postChoose.category}
                </Link>
              ...
              <div className="flex items-center gap-4 p-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 w-fit">
                <img
                  alt={postChoose.author.name}
                  className="w-14 h-14 rounded-full object-cover ring-2 ring-orange-500/50"
                  src={postChoose.author.avatar}
                />

                <div>
                  <p className="font-bold text-white">
                    {postChoose.author.name}
                  </p>

                  <p className="text-sm text-white/60">
                    {postChoose.author.role}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="order-2 lg:order-1">
              ...
              <div className="prose-custom">
                <p className="text-neutral-300 leading-relaxed mb-6 text-lg">
                  {postChoose.content.split(" ").slice(0, 19).join(" ")}
                </p>
                ...
                <p className="text-neutral-300 leading-relaxed mb-6 text-lg">
                  {postChoose.content
                    .split("\n")
                    .find((line) => line.includes("اختر خلفية بسيطة"))}
                </p>
                ...
                >
                  ...
                  {postChoose.content
                    .split("\n")
                    .find((line) => line.includes("## الخلاصة"))
                    .replace("## ", "")}
                </h2>
                ...
                <p className="text-neutral-300 leading-relaxed mb-6 text-lg">
                  {postChoose.content
                    .split("\n")
                    .find((line) => line.includes("البورتريه الناجح يحكي قصة"))}
                </p>
              </div>
